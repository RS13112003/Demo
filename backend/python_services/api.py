...
HF_DATASET_ID = "RS2k03/creditcard-fraud-dataset"
HF_DATASET_FILENAME = "creditcard.csv"

def get_hf_dataset_path() -> str:
    """
    Download/cache the original Hugging Face CSV on the backend.
    """
...
HF_DATASET_URL = os.getenv(
    "HF_DATASET_URL",
    "https://huggingface.co/datasets/RS2k03/creditcard-fraud-dataset/resolve/main/creditcard.csv"
)
...
def get_demo_dataset() -> pd.DataFrame:
    """
    Load the Hugging Face dataset only when a demo endpoint
    actually needs it.

    The dataset is cached in memory after the first successful load.
    """
    global _demo_df
    ...
    try:
        print("Loading demo dataset from Hugging Face...")
        print(f"Dataset URL: {HF_DATASET_URL}")

        _demo_df = pd.read_csv(HF_DATASET_URL)

        print(
            f"Demo dataset loaded successfully: "
            f"{len(_demo_df)} rows, {_demo_df.shape[1]} columns"
        )

        return _demo_df

    except Exception as error:
        print(f"Failed to load Hugging Face dataset: {error}")
        raise RuntimeError(
            f"Unable to load demo dataset from:\n"
            f"{HF_DATASET_URL}\n\n"
...
# ============================================================
# ORIGINAL DATASET — FULL SERVER-SIDE ANALYSIS
# ============================================================

@app.get("/api/analyze-original-dataset")
def analyze_original_dataset():

    CHUNK_SIZE = 10000

    try:
        dataset_path = get_hf_dataset_path()

        print("Original Hugging Face dataset path:")
        print(dataset_path)
        ...
        for chunk in pd.read_csv(
            ...
            if missing_columns:
                raise HTTPException(
                    status_code=400,
                    detail={
                        "message": "Original dataset is missing model features.",
            ...
            if "Class" not in chunk.columns:
                raise HTTPException(
                    status_code=400,
                    detail="Original dataset must contain the Class column.",
                )
            ...
            model_df = chunk[FEATURE_COLUMNS].copy()
            ...
            if model_df.isnull().any().any():
                raise HTTPException(
                    status_code=400,
                    detail="Original dataset contains invalid numeric values.",
                )
            ...
            predicted = fraud_flags.astype(int)
            ...
            high_risk_mask = fraud_flags
            low_risk_mask = ~fraud_flags

            total_high_risk += int(high_risk_mask.sum())
            total_low_risk += int(low_risk_mask.sum())
            ...
            # --------------------------------------------
            # ONLY KEEP REVIEW / HIGH-RISK ROWS
            # --------------------------------------------
            ...
            for local_index in review_indexes:

                row = chunk.loc[local_index]

                result = {
                    "row_number": int(local_index + 2),

                    "fraud_probability": float(
                        probabilities[chunk.index.get_loc(local_index)]
                    ),

                    "fraud_percentage": float(
                        probabilities[chunk.index.get_loc(local_index)] * 100
                    ),
            ...
            print(
                f"Processed {total_transactions:,} rows..."
            )
        ...
        summary = {
            ...
            "review_required": total_high_risk,
            "allow": total_low_risk,
        }
        ...
        print(
            "Original dataset analysis error:",
            repr(error)
        )

        raise HTTPException(
            status_code=500,
            detail=f"Unable to analyze original dataset: {error}",
        )
